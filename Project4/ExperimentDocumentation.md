...
对于消息 $M$ 知道对应的哈希值 $h$ 和填充的长度 $l$，设 $N$ 为拼接新数据的值

以哈希值 $h$ 作为初始IV，计算消息填充的值 $P$ , 然后计算  $CF(h,P||N)$ 就能够得到 $M||P||N$ 的哈希值
...
根据RFC6962构建Merkle树（10w叶子节点），叶子节点前缀0x00，内部节点前缀0x01

### 存在性证明

#### 生成证明

从目标叶子节点开始，逐层向上遍历 Merkle 树，收集每个层级中目标节点的 “兄弟节点哈希” 和 “兄弟节点位置”（左或右）。

重复上述过程，直到遍历至树根，收集的所有兄弟哈希和位置信息即构成证明路径。
#### 验证证明
从目标叶子节点的哈希开始，结合证明路径中的兄弟哈希和位置信息，逐层向上计算父节点哈希。

最终计算结果若与树根哈希一致，则证明目标节点存在于树中。

### 不存在性证明

#### 生成证明
寻找邻居：找到目标索引的 “左邻居”（小于目标索引的最大存在节点）和 “右邻居”（大于目标索引的最小存在节点）。

若目标索引超出树的范围（如大于最大叶子索引），左邻居为最后一个叶子节点，右邻居为空。

若目标索引在树的范围内但无对应节点（如叶子索引是连续的 0~99999，目标为 100001），左邻居为 99999，右邻居为空。

生成邻居的存在性证明：分别为左、右邻居生成存在性证明（同上述 “存在性证明” 过程）。
#### 验证证明
验证左邻居和右邻居的存在性证明（确保它们确实在树中）。

检查位置关系：左邻居索引 < 目标索引 < 右邻居索引，且左邻居索引 + 1 = 右邻居索引（确保两者相邻，中间无其他节点）。

处理边界情况：若目标索引是最小值（如 0）且无左邻居，需验证右邻居存在且索引为 1;
若目标索引是最大值（如 100000）且无右邻居，需验证左邻居存在且索引为 99999。
